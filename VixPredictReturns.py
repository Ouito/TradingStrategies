...
def initialize(context):
    ...
    context.index_VIX=symbol('IND,VIX,USD') # Define the index
    request_data(realTimePrice=[symbol('IND,VIX,USD')])
...
def handle_data(context, data):
    ...
    if sTime.weekday()<=4:
        ...
        if sTime.hour==15 and sTime.minute==59 and context.run_once==False:
            request_data(historyData=[(context.index_VIX, '1 day', '732 D') ])
            currentVIX=show_real_time_price(context.index_VIX, 'last_price')
            twoYearMax=max(data[context.security].hist['1 day']['close']) 
            twoYearMin=min(data[context.security].hist['1 day']['close'])             
